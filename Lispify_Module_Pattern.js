...
*/
	...
	lispInterpreter.Parse = function(input) {
		...
		var ch;
		var that = this;
		this.globalVar.errorFlag = false;
		...
		while(true){		
			...
			if(same('(' , ch)){
				...
				if(same(')' , ch) && !this.globalVar.errorFlag){
		...
		function parseE(){								
			...
			function parseExpression(){			
				...
				else if(operationSelector(ch) == "operator" && !that.globalVar.errorFlag){
	...
	lispInterpreter.Lispify = function(program) {
		...
		var output = this.Evaluate(this.Parse(program));
...
Lispify_Module.Lispify(input1);
console.log("Output: " + Lispify_Module.Lispify(input2));
